  ...
  return (
    <nav style={{ position: 'sticky', top: 0, zIndex: 20, background: 'rgba(238,240,242,.86)', backdropFilter: 'blur(12px)', borderBottom: '1px solid var(--border-hair-soft)' }}>
      <div style={{ ...wrap, padding: '20px 40px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <a href="#top" style={{ textDecoration: 'none' }}><BrandMark name="NEXMIND" /></a>
        <div style={{ display: 'flex', gap: 36, fontSize: 14, fontWeight: 500, color: 'var(--color-muted)' }}>
          {links.map(([t, h]) => (
            <a key={h} href={h} style={{ color: 'inherit', textDecoration: 'none' }}
              onMouseEnter={(e) => (e.currentTarget.style.color = 'var(--color-slate-ink)')}
              onMouseLeave={(e) => (e.currentTarget.style.color = 'var(--color-muted)')}>{t}</a>
          ))}
        </div>
...
export function Delivery() {
  ...
  return (
    <section id="delivery" style={{ paddingBottom: 104 }}>
      <div style={wrap}>
        <SectionHeading eyebrow="Delivery Path" title="用清晰交付节奏，降低 AI 项目的不确定性。" style={{ marginBottom: 56 }} />
  ...
  return (
    <footer style={{ borderTop: '1px solid var(--border-hair)', padding: '48px 0' }}>
      <div style={{ ...wrap, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <BrandMark name="NEXMIND AI" size={12} />
